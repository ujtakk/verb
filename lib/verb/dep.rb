module Verb
  ...
  class Depend < Thor::Group
    class_option :make, type: :boolean, default: true, desc: "generate Makefile (default)"
    class_option :dc, type: :boolean, default: false, desc: "design compiler readfile script"
    class_option :dot, type: :boolean, default: false, desc: "generate dot file"
    ...
    def exists?
      names.map{|top|
        if Dir.glob("#{top}*").empty?
          abort "ERROR: given source (#{top}) does not exist."
        end
      }
    end

    def walk
      Verb::VFile.read_ignore_list("dep.in")

      files_all = []
      @top_vfiles = names.map{|top| Verb::VFile.new(top)}
    ...
    def parse
      mode = if options[:dc]     then :dc
             elsif options[:dot] then :dot
             else                     :make end

      if mode == :make
        print "## -*- makefile -*-\n"
        print Verb::VFiles.new(@files.dup).to_s
        print Verb::VHFiles.new(@files.dup).to_s
        print @top_vfiles_s * "\n"
      elsif mode == :dc
